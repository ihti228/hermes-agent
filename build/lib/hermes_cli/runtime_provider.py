...
def _config_base_url_trustworthy_for_bare_custom(cfg_base_url: str, cfg_provider: str) -> bool:
    ...
    try:
        ...
        if _resolve_provider(cfg_provider_norm) in ("custom", "ollama-local"):
            return True
    except Exception:
        pass
    ...
    *,
    ...
    if requested_norm and requested_norm != "custom":
        try:
            ...
            _resolved = _resolve_provider(requested_norm)
            if _resolved in ("custom", "ollama-local"):
...
def resolve_runtime_provider(
    *,
    ...
    # ollama-local behaves like custom at runtime — it uses config base_url
    # and needs no API key. Normalise so all custom-path logic applies.
